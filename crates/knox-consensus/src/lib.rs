...
impl Ult7RockLattice {
    ...
    fn on_proposal(&mut self, block: Block) -> ConsensusOutput {
        ...
        if block.header.height != self.height {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: expected height {}",
        ...
        if block.header.round != self.round {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: expected round {}",
        ...
        if self.validators.validators.is_empty() {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: validator set is empty",
        ...
        let Some(proposer_idx) = self.validators.index_of_id(&block.header.proposer) else {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: proposer id not in validator set",
                block.header.height, block.header.round
            );
            return out;
        };
        let Some(proposer_pk) = self.validators.validators.get(proposer_idx as usize) else {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: proposer index missing",
                block.header.height, block.header.round
            );
            return out;
        };
        ...
        if !verify_consensus(proposer_pk, &signer_msg.0, &block.proposer_sig) {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: invalid proposer signature",
                block.header.height, block.header.round
            );
            return out;
        }
        ...
        if proposer_idx != leader {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: proposer mismatch",
                block.header.height, block.header.round
            );
            return out;
        }
        if self.banned.contains(&leader) {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: block proposer is slashed",
                block.header.height, block.header.round
            );
            return out;
        }
        ...
        if !verify_block_proof(&block.header, &block.lattice_proof) {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: invalid lattice proof",
                block.header.height, block.header.round
            );
            return out;
        }
        ...
        {
            eprintln!(
                "[StarForge] reject proposal h={} r={}: duplicate lattice contribution",
                block.header.height, block.header.round
            );
            return out;
        }
